...
public class Filter {
  ...
  public String toQueryParameters() {
    return conditions.entrySet().stream().map(entry -> entry.getKey() + "=" + entry.getValue())
        .collect(joining("&", "?", ""));
  }
  ...
  public static class FilterBuilder {
    ...
    public Filter build() {
      ...
      if (!isEmpty(orderBy))
        builder.put("orderBy", "\"" + orderBy + "\"");
      if (!isEmpty(limitToFirst))
        builder.put("limitToFirst", String.valueOf(limitToFirst));
      if (!isEmpty(limitToLast))
        builder.put("limitToLast", String.valueOf(limitToLast));
      if (!isEmpty(startAt))
        builder.put("startAt", startAt);
      if (!isEmpty(endAt))
        builder.put("endAt", endAt);
      if (!isEmpty(equalTo))
        builder.put("equalTo", equalTo);
